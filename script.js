document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getOrCreateError(field) {
      const fieldWrapper = field.closest('.form-group, .field-wrapper, [data-field]') || field.parentElement;
    ...
    function validateField(field) {
      ...
      if (isRequired && !validators.required(value)) {
        errorMessage = field.dataset.errorRequired || errorMessages.required;
      } else if (value.trim() !== '') {
        if ((type === 'email' || field.dataset.validate === 'email') && !validators.email(value)) {
          errorMessage = field.dataset.errorEmail || errorMessages.email;
        } else if ((type === 'tel' || field.dataset.validate === 'phone') && !validators.phone(value)) {
          errorMessage = field.dataset.errorPhone || errorMessages.phone;
        } else if (minLength && !validators.minLength(value, minLength)) {
          errorMessage = field.dataset.errorMinlength || errorMessages.minLength;
        }
      }
  ...
  function applyScrollAnimations() {
    ...
    animatableElements.forEach(function (el, index) {
      const parent = el.closest('[' + STAGGER_ATTRIBUTE + ']');
      if (parent && !el.dataset.delay) {
        const staggerDelay = parseInt(parent.getAttribute(STAGGER_ATTRIBUTE), 10) || 100;
        const siblings = Array.from(parent.querySelectorAll('.fade-in, .slide-up, .slide-in, [data-animate]'));
        const siblingIndex = siblings.indexOf(el);
        if (siblingIndex !== -1) {
          el.dataset.delay = siblingIndex * staggerDelay;
        }
      }
      observer.observe(el);
    });
  ...
  const mutationObserver = new MutationObserver(function (mutations) {
    ...
    if (hasNewNodes) applyScrollAnimations();
  });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "SiteAI משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'adc4ae76-997d-4c24-9d9f-639547e99454',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'adc4ae76-997d-4c24-9d9f-639547e99454';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'adc4ae76-997d-4c24-9d9f-639547e99454';
